...
class LSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, dropout):
        ...
        # Initialize lists to hold parameters for each layer
        ...
        # For the first layer
        self.W_ih.append(nn.Parameter(torch.Tensor(4 * hidden_dim, input_dim)))
        self.W_hh.append(nn.Parameter(torch.Tensor(4 * hidden_dim, hidden_dim)))
        self.b_ih.append(nn.Parameter(torch.Tensor(4 * hidden_dim)))
        self.b_hh.append(nn.Parameter(torch.Tensor(4 * hidden_dim)))

        # For subsequent layers
        for _ in range(1, num_layers):
            self.W_ih.append(nn.Parameter(torch.Tensor(4 * hidden_dim, hidden_dim)))
            self.W_hh.append(nn.Parameter(torch.Tensor(4 * hidden_dim, hidden_dim)))
            self.b_ih.append(nn.Parameter(torch.Tensor(4 * hidden_dim)))
            self.b_hh.append(nn.Parameter(torch.Tensor(4 * hidden_dim)))
    ...
    def reset_parameters(self):
        ...
        for param_list in [self.W_ih, self.W_hh, self.b_ih, self.b_hh]:
    ...
    def forward(self, x):
        ...
        # Initialize lists to store gates, hiddens, cells for each layer
        self.gates = [[] for _ in range(self.num_layers)]
        self.hiddens = [[h_t[layer]] for layer in range(self.num_layers)]
        self.cells = [[c_t[layer]] for layer in range(self.num_layers)]

        for t in range(seq_len):
            layer_input = x[:, t, :]  # Input for the first layer at time t

            for layer in range(self.num_layers):
                # Compute gates
                gates = (torch.mm(layer_input, self.W_ih[layer].t()) + self.b_ih[layer]) + \
                        (torch.mm(h_t[layer], self.W_hh[layer].t()) + self.b_hh[layer])
                ...
                # Store gates, hiddens, cells
                self.gates[layer].append({'i': i_t, 'f': f_t, 'g': g_t, 'o': o_t})
                self.hiddens[layer].append(h_t[layer])
                self.cells[layer].append(c_t[layer])
        ...
        final_out = self.batch_norm(h_t[-1])
    ...
    def lrp(self, R_output, eps=1e-6, bias_factor=0.0):
        ...
        seq_len = len(self.gates[0])  # Number of time steps
        hidden_dim = self.hidden_dim
        input_dim = self.input_dim

        # Relevance scores for hidden and cell states for each layer
        R_h = [ [torch.zeros(batch_size, hidden_dim, device=R_output.device) for _ in range(seq_len + 1)] for _ in range(self.num_layers) ]
        R_c = [ [torch.zeros(batch_size, hidden_dim, device=R_output.device) for _ in range(seq_len + 1)] for _ in range(self.num_layers) ]

        # Start from the output relevance
        h_T = self.hiddens[-1][-1]  # Last hidden state of the last layer
        ...
        R_h[-1][-1] = lrp_linear(h_T, w.t(), b, fc_output, R_output, bias_nb_units=hidden_dim, eps=eps, bias_factor=bias_factor)

        # Initialize relevance for inputs
        R_x = [torch.zeros(batch_size, input_dim, device=R_output.device) for _ in range(seq_len)]

        # Backward relevance propagation through time and layers
        for l in reversed(range(self.num_layers)):
            for t in reversed(range(seq_len)):
                gates_t = self.gates[l][t]
                i_t = gates_t['i']
                f_t = gates_t['f']
                g_t = gates_t['g']
                o_t = gates_t['o']
                c_t = self.cells[l][t + 1]
                c_prev = self.cells[l][t]
                h_prev = self.hiddens[l][t]

                # Relevance from the output gate to the cell state
                R_c_t = R_h[l][t + 1] * o_t * (1 - torch.tanh(c_t) ** 2)
                R_c[l][t + 1] += R_c_t

                # Relevance through the cell state
                R_c_t_total = R_c[l][t + 1]
                ...
                # For g_t
                if l == 0:
                    # Input layer: input is x[:, t, :]
                    hin = torch.cat([h_prev, self.x[:, t, :]], dim=1)  # Shape: (batch_size, hidden_dim + input_dim)
                    w_combined = torch.cat([self.W_hh[l][2 * hidden_dim:3 * hidden_dim, :], self.W_ih[l][2 * hidden_dim:3 * hidden_dim, :]], dim=1)
                    b_combined = self.b_hh[l][2 * hidden_dim:3 * hidden_dim] + self.b_ih[l][2 * hidden_dim:3 * hidden_dim]
                else:
                    # Hidden layers: input is h_t from previous layer
                    hin = torch.cat([h_prev, self.hiddens[l - 1][t + 1]], dim=1)
                    w_combined = torch.cat([self.W_hh[l][2 * hidden_dim:3 * hidden_dim, :], self.W_ih[l][2 * hidden_dim:3 * hidden_dim, :]], dim=1)
                    b_combined = self.b_hh[l][2 * hidden_dim:3 * hidden_dim] + self.b_ih[l][2 * hidden_dim:3 * hidden_dim]
                ...
                Rin_g = lrp_linear(hin, w_combined.t(), b_combined, hout, Rout, bias_nb_units=hin.size(1), eps=eps, bias_factor=bias_factor)

                # Split Rin_g into relevance for h_prev and input
                R_h[l][t] += Rin_g[:, :hidden_dim]
                if l == 0:
                    # For the first layer, accumulate relevance for input x
                    R_x[t] += Rin_g[:, hidden_dim:]
                else:
                    # For higher layers, pass relevance to lower layer's hidden state
                    R_h[l - 1][t + 1] += Rin_g[:, hidden_dim:]

                # Relevance for cell state at previous time step
                R_c[l][t] += R_f
...
dropout = 0.5
num_epochs = 500
vis_prefix = '128hidden_2layers_4decay'
